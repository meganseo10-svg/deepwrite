...
describe("avgScore", () => {
  it("동일 점수의 평균", () => {
    expect(
      avgScore({ lexis: 80, collocation: 80, structure: 80, grammar: 80, tone: 80 }),
    ).toBe(80);
  });
  it("혼합 점수 평균", () => {
    expect(
      avgScore({ lexis: 70, collocation: 80, structure: 90, grammar: 60, tone: 100 }),
    ).toBe(80);
  });
  it("반올림", () => {
    expect(
      avgScore({ lexis: 70, collocation: 70, structure: 70, grammar: 70, tone: 71 }),
...
describe("scoreDimLabel", () => {
  it("정의된 차원은 한글 라벨", () => {
    expect(scoreDimLabel("tone")).toBe("톤");
    expect(scoreDimLabel("grammar")).toBe("문법");
